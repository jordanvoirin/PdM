...
\centering
    ...
    \end{subfigure}
    \decoRule
    \caption{Zernike coefficient as a function of $j$. Noise level is 0.001. The wavefront rms error is 30 nm. The blue line are the true $a_j$'s . The red lines correspond to 100 different retrievals computed with 100 noisy PSFs. The boxplots sum up the 100 retrievals.}
\end{figure}

Figure \ref{fig:ajs_js_jmax_30_WoutN_rmsWFe_30} and \ref{fig:ajs_js_jmaxstudy_WoutN_rmsWFe_30} show results of retrieval done with noiseless PSFs. But given that the final aim is to use this algorithm to characterize the static aberrations present on the optical path to a scientific detector, the PSFs will have noise due to the detector. To be as close to reality as possible with the simulated PSFs, we need to introduce noise into them. We assume that the noise is white Gaussian as it simpler than adding Poisson noise. The random introduction of noise is done with a \verb!numpy! function called \verb!numpy.random.normal()!\footnote{\url{https://docs.scipy.org/doc/numpy/reference/generated/numpy.random.random.html}}.

Figure \ref{subfig:ajs_js_WthE_rmsWFe_30} and \ref{subfig:bxp_ajs_js_rmsWFe_30} presents the results of 100 phase retrievals computed with 100 different noisy PSFs of the same optical system. The comparison between the true Zernike coefficient and the retrieved ones is good but the performance of the algorithm is decreased due to the noise. The mean RMSE of the 100 retrievals, that equals 8.324 nm, is $\sim10$ times bigger than without noise. However, the boxplots in Figure \ref{subfig:bxp_ajs_js_rmsWFe_30} shows that the spread due to the noise is clearly less than 1 nm. The results are similar for other wavefront rms error and different $j_{max}$. 