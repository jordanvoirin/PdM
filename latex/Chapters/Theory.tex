...
In presence of aberrations, which will be discussed in the section \ref{sec:Aberrations}, the wavefront is deformed with respect to the perfect planar or spheric form. The phasor at the exit pupil characterizing a point source is modified as following,

\begin{equation}
U(\xi,\eta) = P(\xi,\eta) e^{j\phi_{Ab}(\xi,\eta)},
\label{eqt:aberratedPhasor}
\end{equation}

where $\phi_{Ab}(\xi,\eta)$ is the dephasing caused by the aberration present between the object plane and the image plane.
