...
def f2j(j,pupilRadius,size,pixelSize):
    ...
    ZjNorm = Zj.*np.conjugate(Zj)
    FFTZjNorm = np.fft.fft2(ZjNorm)
    FFTZjsquare = np.fft.fft2(Zj.*Zj)

    #2Dfft of pupil function
    grid_mask = (im.mk_rad_mask(2*rad)) <= 1
    pupil = np.ones((2*rad,2*rad))*grid_mask
    FFTPupil = np.fft.2fft(pupil)

    return FFTZjNorm - FFTPupil.*FFTZjsquare

def f3j(j,pupilRadius,size,pixelSize,deltaPhi):
    dxp = 1./pixelSize
    rad = np.ceil(pupilRadius/dxp)
    #Get the jth zernike polynomials values on a circular pupil of radius rad
    zernike = zern.calc_zern_basis(1,rad,j)
    ...
    FFTcosZj = np.fft.fft2(cosZj)
    FFTsinZj = np.fft.fft2(sinZj)
    ...
    FFTPupilSin = np.fft.2fft(pupilSin)
    FFTPupilCos = np.fft.2fft(pupilCos)
